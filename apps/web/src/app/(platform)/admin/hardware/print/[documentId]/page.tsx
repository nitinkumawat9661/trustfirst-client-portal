...
export default async function HardwarePrintPreviewPage({
  ...
  const documentTitle = `${projection.document.documentNumber} - ${customerName}`;
      ...
      <section className="print-sheet mx-auto w-full max-w-[210mm] bg-white p-5 shadow-md sm:p-8 print:max-w-none print:p-0 print:shadow-none">
        <style>{`
          @media print {
            ...
            .print-sheet { width: 100% !important; max-width: none !important; min-height: 0 !important; height: auto !important; margin: 0 !important; overflow: visible !important; }
            .print-table-wrap { overflow: visible !important; }
            .print-table { width: 100% !important; min-width: 0 !important; table-layout: fixed !important; font-size: 9px !important; }
            .print-table th, .print-table td { padding-left: 1.2mm !important; padding-right: 1.2mm !important; white-space: normal !important; overflow-wrap: anywhere !important; }
        ...
        <div className="no-print mb-4 flex flex-wrap items-center justify-between gap-3 rounded-md border border-zinc-300 bg-zinc-50 px-3 py-2 text-sm">
          <div>
            ...
            <p className="text-xs text-zinc-600">Thermal formats are disabled. PDF name: {documentTitle}</p>
          </div>
        ...
        <div className="print-table-wrap mt-4 overflow-x-auto">
          <table className="print-table w-full min-w-[720px] border-collapse text-[10px] sm:text-xs print:min-w-0">
            <colgroup>
              <col style={{ width: "4%" }} /><col style={{ width: "31%" }} /><col style={{ width: "9%" }} /><col style={{ width: "6%" }} /><col style={{ width: "7%" }} /><col style={{ width: "10%" }} /><col style={{ width: "7%" }} /><col style={{ width: "10%" }} /><col style={{ width: "6%" }} /><col style={{ width: "10%" }} />
            </colgroup>
            <thead>
              <tr className="border-y border-zinc-500 bg-zinc-100 text-left">
                <th className="px-2 py-2">#</th><th className="px-2 py-2">Item</th><th className="px-2 py-2">HSN</th><th className="px-2 py-2 text-right">Qty</th><th className="px-2 py-2">Unit</th><th className="px-2 py-2 text-right">Rate</th><th className="px-2 py-2 text-right">Disc.</th><th className="px-2 py-2 text-right">Taxable</th><th className="px-2 py-2 text-right">GST</th><th className="px-2 py-2 text-right">Total</th>
              </tr>
            </thead>
            <tbody>
              {projection.items.map((item, index) => (
                <tr className="border-b border-zinc-300 align-top" key={`${item.description}-${index}`}>
                  <td className="px-2 py-2">{index + 1}</td><td className="break-words px-2 py-2 font-medium">{item.description}</td><td className="px-2 py-2">{item.hsnCode ?? "Pending"}</td><td className="px-2 py-2 text-right">{item.quantity}</td><td className="px-2 py-2">{item.unitCode ?? "-"}</td><td className="px-2 py-2 text-right">{money(item.unitAmountCents)}</td><td className="px-2 py-2 text-right">{item.discountPercent === null ? money(item.discountCents) : `${item.discountPercent}%`}</td><td className="px-2 py-2 text-right">{money(item.taxableCents)}</td><td className="px-2 py-2 text-right">{item.taxRateBps / 100}%</td><td className="px-2 py-2 text-right font-medium">{money(item.lineTotalCents)}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <footer className="print-break-avoid mt-10 grid gap-10 border-t border-zinc-400 pt-5 text-xs sm:grid-cols-2">
          <div><p className="font-semibold">Terms</p><p className="mt-1 leading-5">{projection.firm.termsFooter ?? "WAITING FOR CLIENT CONFIRMATION"}</p>{projection.firm.legalName ? <p className="mt-3">Legal proprietor: {projection.firm.legalName}</p> : null}</div>
          <div className="text-right"><p>For {projection.firm.firmName}</p><div className="ml-auto mt-14 w-52 border-t border-zinc-700 pt-2">{projection.signatureLabel}</div></div>
        </footer>
      </section>
    </main>
  );
...
async function loadProjection(service: HardwareTradeService, input: { documentId: string; tenantId: string; userId: string }): Promise<HardwarePrintProjection> {
  try {
    return await service.printProjection({ tenantId: input.tenantId, userId: input.userId }, input.documentId);
  } catch {
    notFound();
  }
}
