      ...
      <CardContent>
        {parties.length === 0 ? <p className="rounded-md border border-dashed border-border p-8 text-center text-sm text-muted-foreground">No party balances are available.</p> : (
          <div className="overflow-x-auto">
            <table className="w-full min-w-[680px] text-left text-sm"><thead className="border-b text-xs uppercase text-muted-foreground"><tr><th className="py-2">Party</th><th>Opening</th><th>Current</th><th>Dr/Cr</th><th>Ledger</th><th>Payment</th></tr></thead><tbody className="divide-y divide-border">{parties.map((party) => <tr key={party.id}><td className="py-3 font-medium">{party.name}</td><td>{money(party.openingBalanceCents)}</td><td>{money(party.currentBalanceCents)}</td><td>{party.balanceSide ?? "Settled"}</td><td><Link className="font-medium text-primary hover:underline" href={`/admin/hardware/ledger?tab=${role}&party=${party.id}`}>Open</Link></td><td><Link className="font-medium text-primary hover:underline" href={`/admin/hardware/payments?tab=${role}`}>Post</Link></td></tr>)}</tbody></table>
          </div>
        )}
      </CardContent>
    </Card>
