  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        <OutstandingTable parties={customers} title="Customer outstanding" />
        <OutstandingTable parties={suppliers} title="Supplier outstanding" />
      </div>
    </div>
  );
}

function OutstandingTable({ parties, title }: { parties: Awaited<ReturnType<HardwareService["listParties"]>>; title: string }) {
  return (
    <Card>
      ...
      <CardContent>
        {parties.length === 0 ? <p className="rounded-md border border-dashed border-border p-8 text-center text-sm text-muted-foreground">No party balances are available.</p> : (
          <div className="overflow-x-auto">
            <table className="w-full min-w-[480px] text-left text-sm"><thead className="border-b text-xs uppercase text-muted-foreground"><tr><th className="py-2">Party</th><th>Opening</th><th>Current</th><th>Dr/Cr</th></tr></thead><tbody className="divide-y divide-border">{parties.map((party) => <tr key={party.id}><td className="py-3 font-medium">{party.name}</td><td>{money(party.openingBalanceCents)}</td><td>{money(party.currentBalanceCents)}</td><td>{party.balanceSide ?? "Settled"}</td></tr>)}</tbody></table>
          </div>
        )}
      </CardContent>
    </Card>
