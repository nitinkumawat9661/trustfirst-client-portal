...
import { HardwareTradeService, type HardwareTradeSummary } from "@/server/hardware";
...
export default async function HardwarePurchasesPage() {
  ...
  const documents: HardwareTradeSummary[] = await service.listPurchases({ tenantId: user.activeTenantId ?? "public", userId: user.id });
  return (
