...
import { InventoryCards, HardwarePluginSummary } from "@/components/hardware/hardware-panels";
...
export default async function HardwareInventoryPage() {
  ...
  const dashboard = await service.dashboard({ tenantId: user.activeTenantId ?? "public", userId: user.id });
  return (
