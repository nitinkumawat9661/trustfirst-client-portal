...
import { hardwareError, hardwarePurchaseDocumentSchema, hardwareResponse, hardwareTradeContext, parseHardwareJson } from "@/server/hardware";
...
export async function POST(request: NextRequest) {
  try {
    const { context, service } = await hardwareTradeContext();
    const input = await parseHardwareJson(request, hardwarePurchaseDocumentSchema);
