...
import {
  commercialDocumentAttachmentSchema,
...
export async function POST(
  ...
  try {
    ...
    const input = await parseCommercialDocumentJson(request, commercialDocumentAttachmentSchema);
    return commercialDocumentResponse(await service.addAttachment(context, documentId, input), 201);
  } catch (error) {
