...
export function PrintButton({ fileName }: { fileName?: string }) {
  ...
  async function printWhenReady() {
    setStatus(fileName ? "Preparing A4 invoice..." : "Preparing print...");
    if (fileName) document.title = fileName;
    ...
    window.print();
    setStatus(fileName ? "Print dialog opened. Save as PDF or print the A4 invoice." : "Print dialog opened. Confirm the printer dialog and check output.");
  }
  ...
  return (
    <div className="flex flex-col items-end gap-1">
      <button className="inline-flex h-9 items-center gap-2 rounded-md bg-zinc-900 px-4 text-sm font-medium text-white" onClick={printWhenReady} type="button">
        <Printer className="size-4" />{fileName ? "Print A4 invoice" : "Print"}
      </button>
