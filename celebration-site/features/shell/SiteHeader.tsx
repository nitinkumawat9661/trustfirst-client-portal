...
export function SiteHeader({ onCreate, showAccount = true }: { onCreate?: () => void; showAccount?: boolean }) {
  ...
  return (
    <>
      <header className="nav">
        <div className="wrap navin">
          ...
          <div className="navAccountActions">
            {showAccount && <Link className="navAccountLink" href={routes.account} aria-label="My Celebration dashboard"><span aria-hidden="true">♡</span><b>My Celebration</b></Link>}
            {onCreate ? <button className="primary navCta" type="button" onClick={onCreate}>{uiContent.nav.create}</button> : <Link className="secondary navStoreLink" href={routes.home}>{uiContent.nav.backToStore}</Link>}
          </div>
        </div>
      ...
      {showAccount && <a className="whatsappDock" href={supportUrl} target="_blank" rel="noreferrer" aria-label="Celebration WhatsApp support"><span aria-hidden="true">WA</span><b>Need help?</b></a>}
    </>
  )
}
