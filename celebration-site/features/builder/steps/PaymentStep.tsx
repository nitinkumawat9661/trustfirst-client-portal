    ...
    <div className="stepPane active">
      ...
      <div className="paymentGrid">
        <div>
          ...
          {showPaymentReturn && !created && <div className="paymentReturnHint" role="status"><span><b>Payment ho gaya?</b> UPI app se wapas aaye ho to transaction UTR/reference yahan paste karein.</span><button className="secondary" type="button" onClick={focusUtr}>UTR field par jao</button></div>}
          ...
          {created && <div className="successBox" data-order-success tabIndex={-1}><strong>{copy.successPrefix} {created.orderId}</strong><span>Order save ho gaya. Ab iska complete progress My Celebration dashboard me bhi dikhega.</span><div className="successActions"><Link className="primary" href={routes.account}>My Dashboard</Link><Link className="secondary" href={created.trackingPath}>{copy.trackingButton}</Link><button className="secondary" type="button" onClick={onWhatsapp}>WhatsApp support</button><button className="secondary" type="button" onClick={onNewOrder}>Ek aur hamper banao</button></div></div>}
          <div className="navRow paymentActions"><button className="secondary" type="button" onClick={onBack}>{copy.back}</button><span className="tiny">{customerAccount ? copy.hint : "Login required only when placing the order"}</span><button className="primary" type="button" disabled={submitting || Boolean(created) || !paymentReady || accountLoading} onClick={onSubmit}>{submitting ? copy.submitting : customerAccount ? "Place order" : "Login & place order"}</button></div>
        </div>
