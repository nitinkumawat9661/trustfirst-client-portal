    ...
    <section id="builder" className="builder">
      <div className="wrap">
        ...
        <div className="builderCard">
          {state.step === 1 && <BudgetStep tierId={state.tierId} occasion={state.checkout.occasion} onTier={state.selectTier} onOccasion={(value) => state.updateField("occasion", value)} onNext={() => state.setStep(2)} />}
          {state.step === 2 && <ProductStep tier={state.tier} products={state.filteredProducts} categories={state.categories} category={state.category} search={state.search} selected={state.selected} pointsUsed={state.pointsUsed} onCategory={state.setCategory} onSearch={state.setSearch} onToggle={state.toggleProduct} onBack={() => state.setStep(1)} onNext={() => state.setStep(3)} />}
