...
export function AdminCatalogManager({ section }: { section: CatalogAdminSection }) {
  ...
  function addOccasion() {
    ...
    edit((current) => ({ ...current, occasions: [...current.occasions, value] }))
  }
  ...
  async function publish() {
    ...
    setError("")
    try {
      const response = await fetch("/api/admin/catalog", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ catalog }) })
      const data = await response.json() as { ok?: boolean; catalog?: CatalogConfig; version?: number; error?: string }
      if (!response.ok || !data.ok || !data.catalog) throw new Error(data.error || "CATALOG_SAVE_FAILED")
      setCatalog(data.catalog)
      setVersion(data.version || version + 1)
      setDirty(false)
      setNotice("Published. Public website ab isi catalog version ko use kar rahi hai.")
    } catch (caught) {
  ...
  return (
    <section className="adminWorkspaceCard">
      <div className="adminWorkspaceHead">
        ...
        <div className="adminActions"><button className="secondary" type="button" onClick={() => { if (!dirty || window.confirm("Unpublished changes discard karne hain?")) load().catch((caught) => setError(String(caught))) }}>Reload</button><button className="primary" type="button" onClick={publish} disabled={busy || !dirty}>{busy ? "Publishing…" : `Publish v${version + 1}`}</button></div>
      </div>
      {dirty && <div className="adminDraftNotice"><b>Draft changes</b> • Public website par abhi v{version} live hai.</div>}
      ...
      {section === "tiers" && <>
        ...
        <div className="adminEditorList">
          {catalog.tiers.map((tier, index) => <div className="adminEditorCard adminDraggableCard" draggable key={`${tier.id}-${index}`} onDragStart={() => setDragIndex(index)} onDragOver={(event) => event.preventDefault()} onDrop={() => { if (dragIndex !== null) edit((current) => ({ ...current, tiers: moveItem(current.tiers, dragIndex, index) })); setDragIndex(null) }}>
      ...
      {section === "products" && <>
        ...
        <div className="adminEditorList">
          {catalog.products.map((product, index) => <div className="adminEditorCard adminDraggableCard" draggable key={`${product.id}-${index}`} onDragStart={() => setDragIndex(index)} onDragOver={(event) => event.preventDefault()} onDrop={() => { if (dragIndex !== null) edit((current) => ({ ...current, products: moveItem(current.products, dragIndex, index) })); setDragIndex(null) }}>
            <div className="adminCardHandle">⋮⋮ <span>Drag to reorder</span></div>
            <div className="adminEditorGrid">
      ...
      {section === "occasions" && <>
        <div className="adminEditorGrid adminDefaultsGrid">
          <label>Default hamper<select value={catalog.settings.defaultTierId} onChange={(e) => edit((current) => ({ ...current, settings: { ...current.settings, defaultTierId: e.target.value } }))}>{catalog.tiers.filter((item) => item.active !== false).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
          <label>Default occasion<select value={catalog.settings.defaultOccasion} onChange={(e) => edit((current) => ({ ...current, settings: { ...current.settings, defaultOccasion: e.target.value } }))}>{catalog.occasions.map((item) => <option key={item}>{item}</option>)}</select></label>
          <label>All-category label<input value={catalog.settings.allCategory.label} onChange={(e) => edit((current) => ({ ...current, settings: { ...current.settings, allCategory: { ...current.settings.allCategory, label: e.target.value } } }))} /></label>
        </div>
        <div className="adminEditorList compact">
          {catalog.occasions.map((occasion, index) => <div className="adminOccasionRow" key={`${occasion}-${index}`}><input value={occasion} onChange={(e) => edit((current) => ({ ...current, occasions: current.occasions.map((item, i) => i === index ? e.target.value : item) }))} /><div><button type="button" className="secondary" disabled={index === 0} onClick={() => edit((current) => ({ ...current, occasions: moveItem(current.occasions, index, index - 1) }))}>↑</button><button type="button" className="secondary" disabled={index === catalog.occasions.length - 1} onClick={() => edit((current) => ({ ...current, occasions: moveItem(current.occasions, index, index + 1) }))}>↓</button><button type="button" className="secondary danger" onClick={() => edit((current) => ({ ...current, occasions: current.occasions.filter((_, i) => i !== index) }))}>Delete</button></div></div>)}
        </div>
