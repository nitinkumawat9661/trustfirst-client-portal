...
PART_NAMES = ["part-00.b64", "part-01.b64", "part-02.b64", "part-02b.b64", "part-03.b64", "part-04.b64", "part-05.b64", "part-06.b64"]
parts = [PARTS_DIR / name for name in PART_NAMES]

missing = [str(path.relative_to(ROOT)) for path in parts if not path.is_file()]
if missing:
    raise SystemExit("Missing Estimate Sale patch chunks: " + ", ".join(missing))

contents = [path.read_text(encoding="utf-8").strip() for path in parts]
encoded = "".join(contents)
if len(encoded) != EXPECTED_B64_LENGTH:
    details = ", ".join(f"{path.name}={len(content)}" for path, content in zip(parts, contents, strict=True))
    raise SystemExit(f"Invalid patch payload length: expected {EXPECTED_B64_LENGTH}, got {len(encoded)} ({details})")
...
namespace = {"__file__": str(Path(__file__).resolve()), "__name__": "__estimate_sale_fast_entry_materializer__"}
