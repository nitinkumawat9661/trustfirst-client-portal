...
if command -v pm2 >/dev/null 2>&1; then
  ...
  PORT="$APP_PORT" pm2 start "npm run start --workspace @trustfirst/web -- --hostname 127.0.0.1" --name "$PM2_PROCESS" --time --update-env
  pm2 save
...
ExecStart=$(command -v npm) run start --workspace @trustfirst/web -- --hostname 127.0.0.1
Restart=always
RestartSec=5
