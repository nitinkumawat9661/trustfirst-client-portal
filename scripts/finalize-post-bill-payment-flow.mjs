...
update("apps/web/src/components/hardware/quick-pos-form.tsx", (input) => {
  ...
  source = replaceSection(
    ...
    `            <div className="rounded-md border border-border bg-muted/50 p-3 text-xs text-muted-foreground">
              Payment is confirmed on the next screen after Post bill. The selection controls the customer ledger, payment receipt and outstanding balance.
            </div>
...
update("apps/web/src/components/hardware/estimate-bill-form.tsx", (input) => {
  ...
  source = replaceSection(
    ...
    `          <div className="self-end rounded-md border border-border bg-muted/50 p-3 text-sm text-muted-foreground">
            Paid, unpaid, or partial status is confirmed after Save. The final choice controls the customer ledger, receipt and outstanding balance.
          </div>`,
...
update("apps/web/src/components/hardware/hardware-trade-form.tsx", (input) => {
  ...
  source = replaceOnce(
    ...
    "Purchase document type watcher",
  );
  source = replaceOnce(
    source,
    `        : mode === "purchase" && locations.length === 0
          ...
          : null;`,
    `        : mode === "purchase" && watchedDocumentType !== "PURCHASE_ORDER" && locations.length === 0
          ? "Add at least one stock location before creating a Purchase Entry or Supplier Bill."
          : null;`,
    "Purchase order location guard",
  );
  source = replaceOnce(
    source,
    `          {mode === "purchase" ? (
            <FormField label="Stock location" required>`,
    `          {mode === "purchase" && watchedDocumentType !== "PURCHASE_ORDER" ? (
            <FormField label="Stock location" required>`,
    "Purchase order location field",
  );
  return source;
});
...
describe("post-bill payment accounting confirmation", () => {
  ...
  it("uses the same decision for supplier payable posting", () => {
    ...
    expect(purchaseSource).toContain('watchedDocumentType !== "PURCHASE_ORDER"');
  });
});
`);
